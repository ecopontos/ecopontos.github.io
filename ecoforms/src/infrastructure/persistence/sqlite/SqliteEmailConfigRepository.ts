...
export class SqliteEmailConfigRepository implements EmailConfigRepository {
    ...
    async get(): Promise<EmailConfig | null> {
        const rows = await this.db.query<EmailConfigRow>(
            `SELECT id, smtp_host, smtp_port, smtp_user, CASE WHEN smtp_password != '' THEN '••••••••' ELSE '' END as smtp_password, from_email, from_name, use_tls, enabled, atualizado_em
    ...
    async save(config: EmailConfig): Promise<void> {
        ...
        if (exists.length === 0) {
            ...
            if (row.smtp_password === '••••••••') {
                await this.db.execute(
                    `UPDATE tbl_email_config SET smtp_host = ?, smtp_port = ?, smtp_user = ?,
                     from_email = ?, from_name = ?, use_tls = ?, enabled = ?, atualizado_em = datetime('now') WHERE id = 'default'`,
                    [row.smtp_host, row.smtp_port, row.smtp_user, row.from_email, row.from_name, row.use_tls, row.enabled]
                );
            } else {
                await this.db.execute(
