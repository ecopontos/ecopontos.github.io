...
export class SqliteLogisticsRepository implements LogisticsRepository {
    ...
    async findAllRoteiros(filter?: RoteiroFilter): Promise<Roteiro[]> {
        ...
        const sql = `SELECT *, tipo_residuo_id AS tipoResiduo FROM roteiros WHERE ${conditions.join(' AND ')} ORDER BY nome`;
        return this.db.query<Roteiro>(sql, params);
    }

    async findRoteiroById(id: string): Promise<Roteiro | null> {
        const rows = await this.db.query<Roteiro>('SELECT *, tipo_residuo_id AS tipoResiduo FROM roteiros WHERE id = ? LIMIT 1', [id]);
        return rows[0] || null;
    }
    ...
    async saveRoteiro(roteiro: Roteiro): Promise<void> {
        ...
        if (exists[0]?.count > 0) {
            ...
            await this.db.execute(
                `INSERT INTO roteiros (id, nome, descricao, tipo_residuo_id, periodicidade, turno, base, distrito, situacao, criado_por, criado_em, atualizado_em)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    async findAllExecucoes(filter?: ExecucaoFilter): Promise<ExecucaoColeta[]> {
        ...
        const sql = `
            SELECT e.*, r.nome AS roteiroNome, m.nome AS motoristaNome, a.nome AS ajudanteNome
        ...
        if (exists[0]?.count > 0) {
            await this.db.execute(
                `UPDATE execucao_coleta SET
                    ...
                    veiculo = ?, km_inicial = ?, km_final = ?, observacoes = ?, inicio_em = ?, fim_em = ?
                WHERE id = ?`,
    ...
    async findExecucaoClientes(execucaoId: string): Promise<ExecucaoCliente[]> {
        return this.db.query(`
            SELECT ec.*, c.nome AS clienteNome
    ...
    async saveExecucaoCliente(item: ExecucaoCliente): Promise<void> {
        const now = new Date().toISOString();
        await this.db.execute(
            `INSERT INTO execucao_clientes (id, execucao_id, cliente_id, coleta_realizada, ocorrencia, observacao, horario_visita, latitude, longitude, registrado_por, registrado_em)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ...
            [item.id, item.execucaoId, item.clienteId, item.coletaRealizada,
             item.ocorrencia || null, item.observacao || null, item.horarioVisita || null,
