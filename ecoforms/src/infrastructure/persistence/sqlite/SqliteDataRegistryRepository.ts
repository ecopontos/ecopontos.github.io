...
export class SqliteDataRegistryRepository implements DataRegistryRepository {
    ...
    async findByTipo(tipo: string): Promise<DataRegistryItem[]> {
        const rows = await this.db.query<RegistryRow>(
            `SELECT id, tipo, chave, conteudo, criado_em, atualizado_em FROM registro_dados WHERE tipo = ? ORDER BY criado_em DESC`,
            [tipo],
        );
    ...
    async findByTipoAndConteudo(tipo: string, conteudo: string): Promise<DataRegistryItem[]> {
        const rows = await this.db.query<RegistryRow>(
            `SELECT id, tipo, chave, conteudo, criado_em, atualizado_em FROM registro_dados WHERE tipo = ? AND conteudo = ? ORDER BY criado_em DESC`,
            [tipo, conteudo],
        );
        return rows.map(rowToItem);
    }
    ...
    async findAll(): Promise<DataRegistryItem[]> {
        const rows = await this.db.query<RegistryRow>(
            `SELECT id, tipo, chave, conteudo, criado_em, atualizado_em FROM registro_dados ORDER BY tipo, criado_em DESC`,
        );
        return rows.map(rowToItem);
    }
    ...
    async findById(id: string): Promise<DataRegistryItem | null> {
        const rows = await this.db.query<RegistryRow>(
            `SELECT id, tipo, chave, conteudo, criado_em, atualizado_em FROM registro_dados WHERE id = ? LIMIT 1`,
            [id],
        );
    ...
    async save(item: DataRegistryItem): Promise<void> {
        ...
        const chave = p.chave ?? p.id;
        if (exists.length === 0) {
            await this.db.execute(
                `INSERT INTO registro_dados (id, tipo, chave, conteudo, criado_em, atualizado_em)
                 VALUES (?, ?, ?, ?, COALESCE(?, datetime('now')), datetime('now'))`,
                [p.id, p.tipo, chave, conteudo, p.criadoEm ?? null],
            );
        } else {
            await this.db.execute(
                `UPDATE registro_dados SET tipo = ?, chave = ?, conteudo = ?, atualizado_em = datetime('now') WHERE id = ?`,
                [p.tipo, chave, conteudo, p.id],
            );
        }
    }
