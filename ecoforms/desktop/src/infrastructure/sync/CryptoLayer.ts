...
export class CryptoLayer {
    ...
    /**
     * Best-effort: remove arquivos de chave legados (raw-key store) deixados por
     * versões anteriores. A chave nunca é persistida — só existe em memória após
     * `deriveAndStoreKey` no login.
     */
    private async purgeLegacyStore(): Promise<void> {
        try {
            ...
            const legacyValue = await legacyStore.get('org_crypto_key');
            if (legacyValue !== null && legacyValue !== undefined) {
                await legacyStore.delete('org_crypto_key');
