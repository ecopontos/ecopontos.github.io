...
describe('GetModuleRuntimeUseCase', () => {
  it('retorna módulo quando encontrado', async () => {
    ...
    const result = await uc.execute('teste', 'admin');
    ...
    expect(repo.loadRuntimeDto).toHaveBeenCalledWith('teste', 'admin');
  });

  it('retorna null quando módulo não existe', async () => {
    const repo = makeMockRepo(null);
    const uc = new GetModuleRuntimeUseCase(repo);
    const result = await uc.execute('inexistente', 'admin');

    expect(result).toBeNull();
  });

  it('delega slug e perfil ao repository', async () => {
    const repo = makeMockRepo(sampleModule);
    const uc = new GetModuleRuntimeUseCase(repo);
    await uc.execute('meu-modulo', 'coordenador');

    expect(repo.loadRuntimeDto).toHaveBeenCalledWith('meu-modulo', 'coordenador');
  });
});
